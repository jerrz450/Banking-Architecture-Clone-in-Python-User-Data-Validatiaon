...
import os
import pandas as pd
from country_api import GetData, Vectorize_Dataframe, Get_Slo_Naslovi
...
class UserPromptsGather:
    ...
    def validate_state_county(self, value):
        try:
            if len(value) == 0:
                return None
    ...
    async def vectorize_df(self, df, country, post_code, city, county, state):
        data = await asyncio.gather(Vectorize_Dataframe(df=df, postal_code=post_code, city=city, county=county, state=state, country = country).run_gather_df())
        return data[0]
    ...
    async def main(self, count = 0):
        ...
        while True:
            ...
            self.dict_log["County"] = county
            self.dict_log["State"] =state
            try:
                if country and city and postal_code: 
                    final = await self.vectorize_df(retrieved_data, country = data[0][1], post_code=postal_code, city=city, county=county, state=state)
                    return final
                else:
                    self._clear_screen()
...
# data_dict  = result[0][1].to_dict()
# print(data_dict["placeName"])
# print(data_dict["postalCode"])
